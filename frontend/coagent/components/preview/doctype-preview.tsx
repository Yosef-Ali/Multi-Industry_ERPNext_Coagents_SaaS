...
export function DocTypePreview({ artifact }: DocTypePreviewProps) {
	...
	if (!docType) {
		return <div className="p-4 text-red-500">Failed to parse DocType definition</div>;
	}
	...
	return (
		<div className="p-6 space-y-6">
			...
			<div className="border-b pb-4">
				...
				<p className="text-sm text-muted-foreground mt-1">Module: {docType.module || 'Custom'}</p>
			</div>
							...
							<div className="flex flex-col space-y-1">
								...
								{field.fieldtype === 'Link' && (
									<div className="border rounded px-3 py-2 text-sm bg-gray-50">
										<span className="text-gray-400">Link to {field.options}</span>
									</div>
								)}

								{field.fieldtype === 'Date' && (
									<input type="date" className="border rounded px-3 py-2 text-sm" disabled />
								)}
