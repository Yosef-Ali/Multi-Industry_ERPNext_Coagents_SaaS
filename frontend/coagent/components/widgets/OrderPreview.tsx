...
import { AlertCircle, Pill, FileText, User, Clock } from 'lucide-react';
...
export function OrderPreview({
    ...
    const groupedItems = orderSet.items.reduce((acc, item) => {
                    ...
                    <div>
                        ...
                        <div className="mt-2 space-y-1 text-sm">
                            <div className="flex items-center gap-2 text-gray-600">
                                ...
                                <span>Patient: <span className="font-medium">{orderSet.patient_name}</span> ({orderSet.patient_id})</span>
                            </div>
                            <div className="flex items-center gap-2 text-gray-600">
                                <User className="w-4 h-4" />
                                <span>Ordering: <span className="font-medium">Dr. {orderSet.ordering_physician}</span></span>
                            </div>
                            <div className="flex items-center gap-2 text-gray-600">
                                <Clock className="w-4 h-4" />
                                <span>Date: <span className="font-medium">{new Date(orderSet.order_date).toLocaleString()}</span></span>
                            </div>
                        </div>
                    </div>
                    <div>
                        {getStatusBadge(orderSet.status)}
                    </div>
                </div>
            </div>
