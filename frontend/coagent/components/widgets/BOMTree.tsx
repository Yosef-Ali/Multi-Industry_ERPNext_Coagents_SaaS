...
import { AlertTriangle, CheckCircle, ChevronDown, ChevronRight, Package } from 'lucide-react';
 ...
 * ```
 */
export function BOMTree({ bomData, showStock = false, onComponentClick }: BOMTreeProps) {
	...
	const renderComponent = (component: BOMComponent, level: number = 0) => {
		...
		const hasSubComponents =
			component.is_sub_assembly && component.sub_components && component.sub_components.length > 0;
		...
		return (
			<div key={`${component.item_code}-${level}`} className="border-l-2 border-gray-200">
					...
					<div className="flex items-start gap-3">
						...
						<div className="flex-1 min-w-0">
							<div className="flex items-start justify-between gap-4">
								<div className="flex-1 min-w-0">
									<h4 className="font-semibold text-gray-900 truncate">{component.item_name}</h4>
				...
				{hasSubComponents && isExpanded && (
					<div className="ml-6">
						{component.sub_components?.map((subComp) => renderComponent(subComp, level + 1))}
					</div>
				)}
			</div>
		);
		...
		<Card className="w-full">
			...
			<div className="bg-blue-50 p-4 border-b">
				<div className="flex items-start justify-between">
					<div>
						<h2 className="text-xl font-bold text-gray-900">BOM Explosion: {bomData.item_code}</h2>
				...
				<div className="bg-gray-50 p-4 border-b">
					<div className="grid grid-cols-3 gap-4">
						...
						<div>
							...
							<div className="text-lg font-semibold text-green-600">{stockSummary.available}</div>
						</div>
						<div>
							<div className="text-xs text-gray-500">Insufficient</div>
							<div className="text-lg font-semibold text-red-600">{stockSummary.insufficient}</div>
						</div>
					</div>
