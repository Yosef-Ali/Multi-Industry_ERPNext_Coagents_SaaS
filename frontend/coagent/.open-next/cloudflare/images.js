...
async function fetchImage(fetcher, imageUrl, ctx) {
	...
	if (imageUrl.startsWith('/')) {
		...
		if (
			define_IMAGES_LOCAL_PATTERNS_default.length > 0 &&
			!define_IMAGES_LOCAL_PATTERNS_default.some((p) => matchLocalPattern(p, url2))
		) {
			return getUrlErrorResponse();
		}
...
export { detectContentType, fetchImage, matchLocalPattern, matchRemotePattern };
