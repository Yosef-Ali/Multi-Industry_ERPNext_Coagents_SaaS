				...
				<Card>
					...
					<CardContent className="space-y-4">
						...
						<div className="rounded-lg bg-muted p-4">
							<pre className="text-xs overflow-x-auto">{JSON.stringify(sharedState, null, 2)}</pre>
						</div>
				...
				{state.toolCalls.length > 0 && (
					<Card>
						<CardHeader>
							...
							<CardDescription>AG-UI TOOL_CALL events from the agent</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="space-y-2">
								{state.toolCalls.map((toolCall) => (
									<div key={toolCall.id} className="rounded-lg border p-3 space-y-2">
				...
				<Card>
					...
					<CardContent className="space-y-2">
						...
						<ul className="list-disc pl-6 text-sm text-muted-foreground space-y-1">
							<li>
								<strong>Event Streaming:</strong> Real-time Server-Sent Events (SSE) from agent to
								UI
							</li>
							<li>
								<strong>Message Events:</strong> TEXT_MESSAGE_START, TEXT_MESSAGE_CONTENT,
								TEXT_MESSAGE_END
							</li>
							<li>
